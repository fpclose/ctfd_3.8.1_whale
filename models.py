...
class WhaleContainer(db.Model):
    ...
    team_id = db.Column(None, db.ForeignKey("teams.id"))
    ...
    team = db.relationship(
        "Teams", foreign_keys="WhaleContainer.team_id", lazy="select")
    ...
    def __repr__(self):
        return "<WhaleContainer ID:{0} {1} {2} {3} {4}>".format(self.id, self.team_id, self.challenge_id,
                                                                self.start_time, self.renew_count)

class WhaleFlag(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    ...
    start_time = db.Column(db.DateTime, nullable=False,
                           default=datetime.utcnow)
    flag = db.Column(db.String(128), nullable=False)
    def __init__(self, team_id, challenge_id, start_time, flag):
        self.team_id=team_id
        self.challenge_id=challenge_id
        self.flag=flag
