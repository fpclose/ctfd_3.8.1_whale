...
class FilesystemCacheProvider:
    def __init__(self, app, *args, **kwargs):
        ...
        self.key = 'ctfd_whale_lock-' + str(kwargs.get('team_id', 0))
...
class RedisCacheProvider:
    def __init__(self, app, *args, **kwargs):
        ...
        if not url:
            raise RuntimeError('Redis cache URL is not configured')
        # decode_responses for spop/sadd handling strings
        self.client = redis.Redis.from_url(url, decode_responses=True)

    def clear(self):
        self.client.delete(self.global_port_key)
        self.client.delete(self.global_network_key)

    def add_available_network_range(self, *ranges):
        if ranges:
            self.client.sadd(self.global_network_key, *ranges)

    def get_available_network_range(self):
        value = self.client.spop(self.global_network_key)
        return value

    def add_available_port(self, port):
        self.client.sadd(self.global_port_key, str(port))

    def get_available_port(self):
        value = self.client.spop(self.global_port_key)
        return int(value) if value is not None else None

    def acquire_lock(self):
        lock = self.client.lock(name=self.key, timeout=10)
