...
class DockerUtils:
    ...
    def _create_standalone_container(client, container):
        ...
        client.services.create(
            ...
            name=f'{container.team_id}-{container.uuid}',
            ...
            labels={
                'whale_id': f'{container.team_id}-{container.uuid}'
    ...
    def _create_grouped_container(client, container):
        ...
        network_name = f'{container.team_id}-{container.uuid}'
        ...
        for name, image in images.items():
            if has_processed_main:
                container_name = f'{container.team_id}-{uuid.uuid4()}'
            else:
                container_name = f'{container.team_id}-{container.uuid}'
