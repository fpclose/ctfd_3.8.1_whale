...
class FrpRouter(BaseRouter):
    ...
    def reload(self, exclude=None):
        ...
        for container in DBContainer.get_all_alive_container():
            if container.uuid == exclude:
                continue
            name = f'{container.challenge.redirect_type}_{container.team_id}_{container.uuid}'
            config = {
                'type': self.types[container.challenge.redirect_type],
                'local_ip': f'{container.team_id}-{container.uuid}',
    ...
    def access(self, container: WhaleContainer):
        ...
        elif container.challenge.redirect_type == 'direct-http':
            return f'<a target="_blank" href="http://{get_config("whale:frp_direct_ip_address", "127.0.0.1")}:{container.port}/">题目链接</a>'
        return ''
